...
import io.unity.application.component.middleware.encoding.buffer._
import io.unity.application.component.middleware.encoding.crypto.{RSA, XTEA}
import io.unity.application.model.PinCodeInput.{EnteredPinCode, StoredPinCodeHash}
...
import io.unity.domain.model.{Email, Machine, Password, ScreenResolution}

/**
  * A [[ByteToMessageDecoder]] that translates a stream of bytes to a
  * [[RequestLogin]] command.
  * @author Sino
  */
final class LoginRequestDecoder(archiveCount: Int, keySet: CredentialBlockKeySet) extends ByteToMessageDecoder with RSA with XTEA {
  val NewLoginId = 16
  ...
  override def decode(ctx: ChannelHandlerContext, in: ByteBuf, out: java.util.List[AnyRef]): Unit = {
    ...
    out.add(RequestLogin(LoginRequest(email, password, clientState, nonce, pinCodeInputType, seeds, previousSeeds, checksums, clientVersion, screenResolution, uid, machine)))
  }

  private def toClientState(id: Int) =
    id match {
      case NewLoginId     => ClientState.LoggingIn
